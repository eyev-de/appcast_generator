...
class CreateCommand extends Command {
  final name = "create";
  final description =
      "Creates a new Appcast file with only meta data. To add <items> use the command 'add'.";

  CreateCommand() {
    argParser
      ..addOption('appcast-file', abbr: 'f')
      ..addOption('title', abbr: 't')
      ..addOption('link', abbr: 'l')
      ..addOption('description', abbr: 'd')
      ..addOption('language', defaultsTo: 'en');
  }
  ...
  void run() {
    if (argResults != null) {
      ...
      final String? link = argResults!['link'];
      final String? description = argResults!['description'];
      final String language = argResults!['language'];
      if (path == null)
        throw 'Please provide the name of the Appcast file to be created.';
      if (title == null) throw 'Please provide the title of the Appcast.';
      if (link == null) throw 'Please provide the link to the Appcast.';
      if (description == null)
        throw 'Please provide the description of the Appcast.';
      File file = File(path);
...
class AddCommand extends Command {
  final name = "add";
  final description =
      "Adds a <item> element to an Appcast file. To create a new Appcast use 'create'.";
  ...
  AddCommand() {
    ...
    argParser
      ...
      ..addOption('release-date',
          abbr: 'd', defaultsTo: formatter.format(DateTime.now()))
  ...
  Future<void> run() async {
    if (argResults != null) {
      ...
      if (path == null)
        throw "There must be an input file. Use 'appcast-file' or 'f' and append the path to the inputfile.";
      if (title == null) throw 'Please provide a title for the new version.';
      if (notes == null)
        throw 'Please provide an url to the release notes for the new version.';
      if (url == null)
        throw 'Please provide a url where this update can be downloaded.';
      if (os == null)
        throw 'Please provide for which operatiing system this update is for.';
      if (updateFile == null)
        throw "Please provide the update file so that it can be signed and the content length can be calculated.";
      ...
      await privateKeyFile
          .writeAsString(base64Encode(await keyPair.extractPrivateKeyBytes()));
      ...
      if (str == null)
        throw 'An error occured creating your updated Appcast file.';
      file.writeAsStringSync(str);
    }
  }
}
...
class UpdateCommand extends Command {
  final name = "update";
  final description =
      "Update the <item> element of a single item Appcast file. To create a new Appcast use 'create'.";
...
int main(List<String> arguments) {
  var exitCode = 0;
  var cr = CommandRunner('appcast_generator',
      'appcast_generator generates Appcast files with certain user input as well as files.')
